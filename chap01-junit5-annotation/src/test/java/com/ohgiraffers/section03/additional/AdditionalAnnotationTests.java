...
public class AdditionalAnnotationTests {
    ...
    * */
    ...
    /*
    * 테스트 메소드는 실행 순서가 보장되지 않지만 경우에 따라서는 (통합테스트 환경등)
    * 테스트의 순서가 중요한 경우도 있다
    * 클래스 레벨에서 @TestMethodOrder(MethodOrderer.OrederAnnotation.class)어노테이션을 추가하고
    * 각 테스트 메소드 @Order 어노테이션으로 순서를 지정하면 테스틑 순서를 설정할 수 있다
    * 클래스에 작성한 테스트 메소드의 순서는 MethodOrderer에 displayName, MethodName, OrderAnnotation, Random
    * 등이 있다.
    * */
    @Test
    @Order(1)
    ...
    /*
    * RepeatedTest는 명시된 숫자로 얼마나 테스트를 반복할 것인지 지정해서 사용한다
