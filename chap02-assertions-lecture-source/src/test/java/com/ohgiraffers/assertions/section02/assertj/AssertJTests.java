...
public class AssertJTests {
    ...
    * 개발자가 테스트를 하면서 필요하다고 생각하는 거의 모든 기능을 메소드로 제공한다
    * 참고 hamncrest, truth 등도 사용하는 용법이나 컨셉이 비슷하지만 보통 assertj를 많이 사용
    * 가장 직관적이고 편하며ㅡ junit5 팀에섯도 공식적으로 추천하고 있는 third party lib이다
    *
    * */
    ...
    //날짜 객체가 가지는 년월일시분토 밀리포 나노초 까지도 검증이 가능하며ㅡ 특정 날짜 범위 이전 이후 내용도 검증이 가능
    @Test
    @DisplayName("날짜 테스트하기")
    void testLocalDateTimeValidation(){
        // given
        String birthDay =  "2014-09-18T18:42:00.000";
    ...
    void testExceptionValidation(){
        //given
        //when
        Throwable thrown = AssertionsForClassTypes.catchThrowable(()->{throw new IllegalArgumentException("잘못된 파라파라라라라라");});

        //then
        Assertions.assertThat(thrown)
                .isInstanceOf(IllegalArgumentException.class)
                .hasMessageContaining("파라");
    }
...
//                .hasMessageContaining("입력");
        ...
        * 3. asserThatillegaalStateException
    ...
    void testExtractPropertyTuple() {
        ...
        Assertions.assertThat(members)
                .extracting("name", "id")
                .contains(
                        Tuple.tuple("홍길동", "user01"),
                        Tuple.tuple("유관순", "user02")
                );
    }
}
