...
//        Assertions.assertEquals(expected,result,() -> "실패할 때 보여줄 메세징징이");
...
//        Assertions.assertEquals(result, num2 == num1);
    ...
    void testAssertAll(){
        ...
        Assertions.assertAll(
                () -> Assertions.assertEquals(firstName, person.getFirstName(), "first name 이 잘못됨"),
                () -> Assertions.assertNotEquals(lastName, person.getLastName(), "lastName 이 잘못됨")
        );
    }
...
//        );
        ...
        //then
        Assertions.assertAll(
                ()-> Assertions.assertInstanceOf(PositiveNumberValidator.class, validator), //클레스 비교
                ()-> Assertions.assertInstanceOf(IllegalArgumentException.class, exception), // 예외 비교
                ()-> Assertions.assertDoesNotThrow(() ->validator.checkDividableNumbers(secondNum,firstNum)), // 오류 없는지 비교
                ()-> Assertions.assertEquals(exceptedErrorMessage, exception.getMessage())
        );
    }

    @Test
    @DisplayName("예상 목록이 실제 목록과 일치하지 않는 경우 확인")
